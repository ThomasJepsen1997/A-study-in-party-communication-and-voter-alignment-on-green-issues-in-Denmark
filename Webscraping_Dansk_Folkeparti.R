...
url <- "https://danskfolkeparti.dk/category/kristians-ugebrev/page/"

# create empty data set with titles
cols <- c("titles", "links", "date", "text")
dat_DF <- cols %>% t %>% as_tibble(.name_repair = "unique") %>% `[`(0, ) %>% rename_all(~cols)
...
text_blog = function(link) {
  ...
  blog_text = press_page %>% html_nodes('.entry-content') %>%
    html_text()  
  return(blog_text) }
...
while (T) {
  ...
  file.name <- str_c("./press_archive/", "2021-09-19", " -- Dansk_Folkeparti ", pages, "-", (pages + 1), ".html")
  ...
  titles <- page %>% html_nodes('.entry-title a') %>% html_text(trim = T)
  
  
  
  # pull out links to posts
  links <- page %>% html_nodes('.entry-title a') %>% html_attr("href")
...
save(dat_DF, file = "Dansk_Folkeparti_press.rda")
##########################