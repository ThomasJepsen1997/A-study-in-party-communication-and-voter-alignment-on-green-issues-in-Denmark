...
url <- "https://sf.dk/nyheder/side/"

# create empty data set with titles
cols <- c("titles", "links", "date", "text")
dat_SF <- cols %>% t %>% as_tibble(.name_repair = "unique") %>% `[`(0, ) %>% rename_all(~cols)
...
text_blog = function(link) {
  ...
  blog_text = press_page %>% html_nodes('.col-sm-8 p') %>%
...
while (T) {
  ...
  file.name <- str_c("./press_archive/", "2021-09-19", " -- SF ", pages, "-", (pages + 1), ".html")
  ...
  titles <- page %>% html_nodes('h2 a') %>% html_text(trim = T)
