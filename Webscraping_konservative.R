...
url <- "https://konservative.dk/category/pressemeddelelser/page/"

# create empty data set with titles
cols <- c("titles", "links", "date", "text")
dat_k <- cols %>% t %>% as_tibble(.name_repair = "unique") %>% `[`(0, ) %>% rename_all(~cols)
...
text_blog = function(link) {
  ...
  blog_text = press_page %>% html_nodes('.gdlr-blog-content') %>%
    html_text()  
  return(blog_text) }
...
final.url <- str_c(url, pages)
  ...
  file.name <- str_c("./press_archive/", "2021-09-19", " -- Konservative ", pages, "-", (pages + 1), ".html")
  ...
  titles <- page %>% html_nodes('.gdlr-blog-title a') %>% html_text(trim = T)
  

  
  # pull out links to posts
  links <- page %>% html_nodes('.gdlr-blog-title a') %>% html_attr("href")
  
  # pull out and trim the date of the blogs
  date <- page %>%
    html_nodes('.blog-date a') %>%
    html_text(trim = T)
...
save(dat_k, file = "Konservative_press.rda")
##########################